...
describe("v3 product boxes", () => {
  ...
  it("keeps command suggestions and double escape local to prompt", () => { const onIntent = vi.fn(); const onChange = vi.fn(); render(<PromptBox value="/" onChange={onChange} onIntent={onIntent} />); const prompt = screen.getByLabelText("Search query"); expect(prompt).toHaveClass("prompt-caret-cycle"); expect(screen.getByRole("listbox")).toBeInTheDocument(); fireEvent.keyDown(prompt, { key: "Escape" }); expect(screen.queryByRole("listbox")).not.toBeInTheDocument(); });
  ...
  it("keeps evidence sources in one ordered full-width region", () => { const onIntent = vi.fn(); const second = { ...source, sourceId: "src_2" as never, title: "Second source" }; render(<EvidenceBox sources={[source, second]} onIntent={onIntent} />); const region = screen.getByRole("complementary", { name: "Evidence" }); expect(region.querySelector("ul")?.children).toHaveLength(2); expect(region.querySelectorAll("li")[0]).toHaveTextContent("Source"); expect(region.querySelectorAll("li")[1]).toHaveTextContent("Second source"); });
