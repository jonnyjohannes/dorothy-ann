...
function normalizedResults(raw: unknown, maxResults: number): SearchResult[] {
  ...
  for (const value of raw) {
    ...
    results.push({ ...result, rank } as SearchResult);
  }
...
export async function executeSearchTurn(input: SearchTurnExecutionInput): Promise<SearchTurnExecutionResult> {
  ...
  try {
    ...
    const results = normalizedResults(raw, maxResults);
    ...
    const turn: SearchTurn = results.length > 0
      ...
      : {
          ...
          finishedAt,
          userMessage: input.userMessage,
          result: { completion: "empty", destinations: [] },
        };
