...
export class LocalThreadStore implements ThreadStore {
  ...
  async inspectImport(backup: ThreadBackup): Promise<ImportPreview> { const issues = validateBackup(backup); const db = await this.dbPromise; const existing = new Set(await db.getAllKeys("threads")); const threads = backup && Array.isArray(backup.threads) ? backup.threads : []; const valid = threads.filter((item) => item && typeof item === "object" && validThread((item as { thread?: unknown }).thread)) as Array<{ thread: Thread }>;  return { add: valid.filter(({ thread }) => !existing.has(thread.id)).length, conflicts: valid.filter(({ thread }) => existing.has(thread.id)).length, skippedInvalid: issues.length, issues }; }
  async importData(backup: ThreadBackup, options: { onConflict: "skip" | "replace" }): Promise<ImportReport> { const preview = await this.inspectImport(backup); const added: string[] = []; const replaced: string[] = []; const skipped: string[] = []; const threads = backup && Array.isArray(backup.threads) ? backup.threads : []; if (preview.issues.length && !threads.length) return { ...preview, added, replaced, skipped }; const db = await this.dbPromise; const tx = db.transaction(["threads", "threadSummaries"], "readwrite"); for (const item of threads) { if (!item || typeof item !== "object" || !validThread((item as { thread?: unknown }).thread)) continue; const { thread } = item as { thread: Thread }; const exists = await tx.objectStore("threads").getKey(thread.id); if (exists && options.onConflict === "skip") { skipped.push(thread.id); continue; } await tx.objectStore("threads").put({ schemaVersion: 1, thread }, thread.id); await tx.objectStore("threadSummaries").put(summary(thread), thread.id); (exists ? replaced : added).push(thread.id); } await tx.done; return { ...preview, added, replaced, skipped }; }
}
function validateBackup(backup: ThreadBackup): Array<{ code: "invalid_backup"; message: string }> { if (!backup || backup.backupVersion !== 1 || !Array.isArray(backup.threads)) return [{ code: "invalid_backup", message: "unsupported or malformed backup" }]; return backup.threads.flatMap((item) => item && typeof item === "object" && validThread((item as { thread?: unknown }).thread) ? [] : [{ code: "invalid_backup", message: "thread record failed validation" }]); }
