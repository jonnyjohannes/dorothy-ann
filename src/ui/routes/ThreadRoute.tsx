...
import type { CanonicalSource, Thread, ThreadContext, ThreadId, TurnId, UserMessage } from "../../domain/model-v3";
...
export function ThreadRoute() {
  ...
  const requestedKind = params.get("mode") === "research" || resolveQueryMode(query) === "research" ? "research" : "search";
  ...
  useEffect(() => {
    let cancelled = false;
    void getStore().load(threadId).then((result) => { if (!cancelled && result.ok) setThread(result.value?.thread ?? null); });
    return () => { cancelled = true; controller.current?.cancel("navigation"); };
  }, [threadId]);
  ...
  const run = useCallback(async (request: string, kind: "search" | "research") => {
    ...
    if (result.ok) { setThread(result.record.thread); if (routeThreadId === "new") navigate(`/topics/${encodeURIComponent(String(threadId))}`, { replace: true }); }
    else setMessage(result.error === "commit_retryable" ? "The result was not saved. Retry save." : "That turn could not be completed.");
  }, [navigate, routeThreadId, thread, threadId]);
  ...
  const onIntent = (intent: BoxIntent) => {
    if (intent.type === "prompt_submitted") { setValue(""); void run(intent.value, resolveQueryMode(intent.value) === "research" ? "research" : "search"); }
