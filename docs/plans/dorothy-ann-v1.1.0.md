...
- Current focus: server/browser source-reference policy drift repaired
- Handoff lives in: [`## Handoff`](#handoff)
- Next action: push/deploy the four prior local fixes plus the shared source-reference policy repair, then smoke one live research turn
...
The browser invalid-result root cause is repaired. Server terminal assembly and browser/commit validation had separate research source-reference walkers: the commit/browser policy included `ResearchGap.support`, while `executeResearchTurn` omitted it when selecting terminal `sourceRecords`. A resolution whose source was retained only through ledger-gap support therefore passed server turn-shape validation but failed browser source closure. `collectResearchStateSourceIds` is now the shared resolution/checkpoint policy used by terminal assembly and commit/browser validation, with a direct gap-support regression. A real-boundary regression also executes two consecutive research turns through Hono SSE, the browser gateway/controller, contextual evidence projection, and IndexedDB commit; both complete, and the corresponding Chromium/mobile-WebKit smoke passes on isolated fixture ports. Research terminals are fully schema-validated at the server boundary, and every locally detected controller failure carries a bounded actionable message rather than falling through to `That turn could not be completed.` The preceding closure fixes remain local and unpushed, so a deployment sourced from `origin/release/v1.1.0` cannot contain them until pushed. Verification passes: lint, typecheck, 202 tests, production build, six isolated-port Playwright tests, and `git diff --check`.
