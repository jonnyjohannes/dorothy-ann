...
- Current focus: post-release UX amendment complete — fuller workspace, bottom evidence, and styled brand control
- Handoff lives in: [`## Handoff`](#handoff)
- Next action: none; future visual changes require a new amendment or plan
...
- Evidence presentation is a single bordered region with a top and bottom rule. Sources are laid out as flexible, consistently padded result cells that flow across desktop and wrap naturally on mobile. Source title, URL, and snippet remain accessible and each source keeps its existing activation intent.
- Preserve the existing semantic box boundaries, source identity/order, citation behavior, keyboard/focus behavior, themes, and reduced-motion behavior. The brand control must render as a deliberately styled header control rather than a browser-default button, while retaining its single activation target and accessible name. This is a visual/layout amendment only; no domain, route, storage, or provider contract changes.
