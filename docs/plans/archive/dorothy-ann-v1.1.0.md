...
Earlier completed checkpoints follow. Plan Ledger item 30 established prompt-owned sufficiency thresholds for complex comparative/causal/contested/multi-obligation research. The later root corroboration amendment requires `ASSESSOR.md` to target at least two materially independent sources for every root central conclusion, including straightforward factual and navigational questions, while retaining complete-obligation coverage and four materially independent sources overall for complex requests. Independence remains model-judged: URL/domain count, syndication, repeated reports, and same-publisher pages are not automatic proof. This remains prompt-owned policy; existing application support validation, untrusted-content rules, and all bounded research ceilings remain unchanged. That checkpoint passed lint, typecheck, 219 tests, production build, and `git diff --check`.
