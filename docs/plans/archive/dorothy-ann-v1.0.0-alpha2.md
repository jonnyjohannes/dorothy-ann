...
- The prompt bar has no visible submit button, lookup/research buttons, or selector. Submission is Enter-driven: ordinary input performs lookup; a terminal `?` selects research. The prompt bar keeps a short hint explaining the `?` macro and should receive stronger visual emphasis without rounded borders.
- Settings and Threads use the same secondary-page header: sticky brand/kicker, top-right `×` close action returning home, and Escape-to-close. Threads retains keyboard selection, Enter navigation, explicit Delete confirmation, and now maps the Delete key to the same confirmation flow.
- Research synthesis must begin exactly with `According to my research...`; this is a prompt directive and fixture contract, not a post-hoc UI decoration.
