...
The browser interaction decisions, React + Vite + Hono + Vercel deployment stack, React Router routes, selective XState workflow boundaries, and initial provider split are settled: Brave Search for ranked discovery, application-controlled extraction, and Anthropic for chat/synthesis. The application-owned extraction implementation is settled on `undici` + `linkedom` + Mozilla Readability in the Vercel Node runtime. The remaining implementation-level choice is the durable login-attempt limiter behind the Vercel adapter.
