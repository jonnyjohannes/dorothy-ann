...
Pending: choose the Vite UI framework. React remains the familiar baseline; Vue, Svelte, Solid, and Preact are viable Vite-native alternatives. The choice should optimize the highly interactive authenticated SPA—streaming state, accessible overlays, IndexedDB, Markdown rendering/editing, and testability—rather than SSR or content-site performance. Vercel remains the target regardless of UI choice; SPA deep-link rewrites belong only in `vercel.json`.
...
The browser interaction decisions and Vite + Hono + Vercel deployment stack are settled. Remaining implementation-level choices are the Vite UI framework, concrete chat/search/extraction providers, durable login-attempt limiter behind the Vercel adapter, and supporting UI/test packages.
