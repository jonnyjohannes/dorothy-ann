...
- Current focus: browser topics/evidence are now persisted and visible; continue report workbench, retry semantics, and extractor hardening before final alpha validation
...
- [~] 9. Shell and lookup UI — deliverable: auth shell, drawer, mode routing, lookup/promotion states; current: browser mode routing, local API proxy, lookup/research results, IndexedDB topic summaries, reopen flow, and desktop/mobile evidence presentation wired; remaining: persisted auth shell, promotion action, accessible recovery states, and E2E coverage.
