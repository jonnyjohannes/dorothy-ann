...
- Current focus: local browser lookup/research flow and provider readiness endpoint are wired; ready for real Brave/Anthropic credentials after persistence and extractor hardening continue
...
- [~] 9. Shell and lookup UI — deliverable: auth shell, drawer, mode routing, lookup/promotion states; current: browser mode routing, local API proxy, lookup results, research results, and source links wired; remaining: persisted auth/topic shell, drawer, accessible recovery states, and E2E coverage.
