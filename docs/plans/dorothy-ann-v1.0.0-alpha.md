...
- Current focus: Anthropic adapter is implemented and citation-safe in replay fixtures; next wire provider events into staged orchestration while extractor hardening remains active
...
- [~] 7. Anthropic adapter — deliverable: chat/research/report streaming with validated citation sentinels; current: adapter, evidence envelope, usage normalization, and citation replay fixtures implemented; remaining: provider interruption/injection/persistence-boundary coverage and route integration.
- [~] 8. Orchestration/SSE — deliverable: turn/report streams, bounded extraction, stage-aware retry; current: fixture `/api/turn` lifecycle and terminal-event contract verified; remaining: real staged orchestration/retry/persistence.
