...
- **Activation** — the title and detached thumbnail share the same source-page activation target when available; otherwise both use the canonical media URL. There is no separate `Open image` / `Open video` action. The canonical media URL remains durable identity metadata for future evidence use.
...
- **Portable video player** — owner: product/security — deferred and non-blocking: Brave supplies a source URL, thumbnail, and metadata but no portable iframe/embed URL. Any inline player requires a separate allowlisted provider-specific embed policy, sandbox/privacy contract, and click-to-load decision.
