...
- **Activation** — link and image titles/attachments open the source page when available, otherwise the canonical URL. For playable videos, the title remains the reliable external source-page link while clicking the thumbnail starts the inline player. Unsupported or failed video playback falls back to the linked thumbnail. There is no separate `Open image` / `Open video` action. The canonical media URL remains durable identity metadata for future evidence use.
...
- **Media presentation** — every result kind keeps the same numbered title, constellation accent, source metadata, focus, citation-anchor treatment, and increased title/cell breathing room. Image/video cards use `coalesce(detached media attachment, description snippet)`: when a thumbnail exists it replaces the snippet in a full-card-width detached `16:9` tile; unusual source aspect ratios are centered with `object-fit: contain` rather than cropped or stretched; otherwise the snippet renders normally. Image thumbnails remain source-page links. Supported video thumbnails are ReactPlayer light-mode previews that load and start the player only after explicit activation; nothing autoplays before that action.
...
Add `react-player` as a runtime dependency (initial target `^3.4.0`, lockfile authoritative). Use its URL detection and provider adapters rather than persisting or constructing provider iframe HTML. Only video result cells are eligible. Call `ReactPlayer.canPlay(videoUrl)` before offering inline playback, pass the existing thumbnail through light mode, retain native/provider controls, and load external player code only after explicit thumbnail activation. The title always remains an external source-page link. Unsupported URLs and runtime player errors retain or restore the linked thumbnail fallback. Player readiness, playback, and failure state are ephemeral component state; they never enter turns, source records, storage, transport, or research evidence. Do not autoplay before activation or add global player state.
