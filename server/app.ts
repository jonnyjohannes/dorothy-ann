...
export function createApp({ config, remoteThreads: injectedRemoteThreads }: AppDependencies) {
  ...
  app.get("/api/providers/status", (context) => context.json({ fixtureMode: config.DOROTHY_FIXTURE_MODE, storage: config.DOROTHY_FIXTURE_MODE || Boolean(remoteThreads), search: config.DOROTHY_FIXTURE_MODE || Boolean(searchProvider), chat: config.DOROTHY_FIXTURE_MODE || Boolean(chatProvider), extraction: config.DOROTHY_FIXTURE_MODE || Boolean(extractor) }));
