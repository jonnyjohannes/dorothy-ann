...
export function createApp({ config, systemPrompts, remoteThreads: injectedRemoteThreads, threadStoreV3: injectedThreadStoreV3 }: AppDependencies) {
  ...
  app.put("/api/threads/:threadId", async (context) => {
    ...
    catch (error) { if (error instanceof Error && error.message === "storage_conflict") return context.json({ error: { code: "conflict", message: "thread has changed" } }, 409); if (error instanceof Error && error.message === "invalid_thread_id") return context.json({ error: { code: "invalid_request", message: "thread id is invalid" } }, 400); console.error("remote_thread_commit_failed", error instanceof Error ? error.message : "unknown_error"); return context.json({ error: { code: "storage_error", message: "thread could not be saved" } }, 503); }
  });
