...
export function loadConfig(environment: Record<string, string | undefined> = process.env): AppConfig { if (environment.NODE_TLS_REJECT_UNAUTHORIZED === "0") throw new Error("insecure_tls_configuration"); const parsed = envSchema.safeParse(environment); if (!parsed.success) throw new Error(`invalid configuration: ${parsed.error.issues.map((issue) => issue.path.join(".")).join(", ")}`); return parsed.data; }
