...
import type { ContentExtractor, ExtractionLimits } from "../src/ports/extraction.js";
...
export function assertTlsVerificationEnabled(environment: Record<string, string | undefined> = process.env): void {
  if (environment.NODE_TLS_REJECT_UNAUTHORIZED === "0") {
    throw new Error("insecure_tls_configuration");
  }
}
  ...
  async extract(source: SearchResult, limits: ExtractionLimits): Promise<ExtractionOutcome> {
    ...
    for (let redirects = 0; redirects <= this.config.maxRedirects; redirects++) {
      ...
      try {
        ...
        const buffer = await readBoundedBody(response, this.config.maxFetchBytes, controller.signal);
        const raw = new TextDecoder().decode(buffer);
        const text = contentType === "text/html" ? readableText(raw) : raw;
        const bounded = text.replace(/\s+/g, " ").trim().slice(0, limits.maxCharacters);
